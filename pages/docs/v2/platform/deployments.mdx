...
If you request a **preview URL**, the `X-Robots-Tag` HTTP header will be set as `noindex`. That means preview URLs will NOT be indexed by search engines like Google. Learn more about `X-Robots-Tag` [here](https://developers.google.com/search/reference/robots_meta_tag#xrobotstag).
